...
export default function Post({name, message, profile, photo, time, deletePost }) {

  const [like, setLike] = useState({ color: "gray" });
  const [count, setCount] = useState(230);

  return (
    <div className="bg-white py-2 my-2 w-screen sm:w-[64vw] md:w-[49vw] rounded-[10px] shadow-sm ">
      ...
      <div className="mx-2">
        <div className="my-2 ml-2 text-[14px] font-semibold">
          <p>{message}</p>
        </div>
        {photo && <div className="w-[100%] bg-gray-100">
          <img
            className={!photo ? "hidden" : "w-[100vw] h-[70vh] sm:h-[95vh] object-contain py-1"}
            src={photo}
            alt="post"
          />
        </div>}
      ...
      <div className="flex items-center border-t-[1px] border-gray-200 px-4 my-1 justify-between sm:justify-around space-x-[75px] sm:space-x-0 mx-2">
        <div
        ...
        <div className=" flex items-center py-2 sm:px-8 rounded-md hover:sm:bg-gray-200 my-1 cursor-pointer" onClick={deletePost}>
